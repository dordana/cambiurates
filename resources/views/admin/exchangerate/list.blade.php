                        ...
                        <tr class="footable-{{$iIdx % 2 == 0 ? 'odd' : 'even'}}" style="display: table-row;" id="rate_{{ $oExchangeRate->id }}">
                            ...
                            <td>
                                <input type="text"
                                       pattern="[0-9]"
                                       title="Numbers only"
                                       value="{{ $oExchangeRate->userExchangeRate()->first()->buy or 0 }}"
                                       class="form-control buy col-md-4"
                                       name="buy[]" data-name="buy"
                                       style="width:30%;"
                                        {{ (!$oExchangeRate->userExchangeRate()->first()->type_buy || $oExchangeRate->userExchangeRate()->first()->type_buy === 'disabled') ? 'disabled' : ''}}>
                                <select name="buy_trade[]" data-name="trade_type" data-placeholder="Choose a trade type..." class="chosen-select col-md-6" style="width:40%;" tabindex="4">
                                    <option {{ $oExchangeRate->userExchangeRate()->first()->type_buy === 'disabled' ? 'selected' : '' }} value="disabled">Disabled</option>
                                    <option {{ $oExchangeRate->userExchangeRate()->first()->type_buy === 'percent' ? 'selected' : '' }} value="percent">Margin(%)</option>
                                    <option {{ $oExchangeRate->userExchangeRate()->first()->type_buy === 'fixed' ? 'selected' : '' }} value="flat_rate">Flat Rate</option>
                                </select>
                            </td>
                            <td>
                                ...
                                <input type="text"
                                       pattern="[0-9]"
                                       title="Numbers only"
                                       value="{{ $oExchangeRate->userExchangeRate()->first()->sell or 0 }}"
                                       class="form-control margin-rate-input col-md-4"
                                       name="sell[]" data-name="sell"
                                       style="width:30%;"
                                        {{ (!$oExchangeRate->userExchangeRate()->first()->type_sell || $oExchangeRate->userExchangeRate()->first()->type_sell === 'disabled') ? 'disabled' : ''}}>
                                <select name="sell_trade[]" data-name="trade_type" data-placeholder="Choose a trade type..." class="chosen-select col-md-6" style="width:40%;" tabindex="4">
                                    <option {{ ( $oExchangeRate->userExchangeRate()->first()->type_sell === 'disabled') ? 'selected' : '' }} value="disabled">Disabled</option>
                                    <option {{ ($oExchangeRate->userExchangeRate()->first()->type_sell === 'percent') ? 'selected' : '' }} value="percent">Margin(%)</option>
                                    <option {{ ($oExchangeRate->userExchangeRate()->first()->type_sell === 'fixed') ? 'selected' : '' }} value="flat_rate">Flat Rate</option>
                                </select>
                            </td>
                            <td>
                                <div class="flag-toggle-2">
                                    <label class="checkbox-inline">
                                        <input type="checkbox" class="visible-switch"
                                               {{ ($oExchangeRate->userExchangeRate()->first()->visible == 1) ? 'checked' : '' }}
                                               data-id="{{ $oExchangeRate->userExchangeRate()->first()->id }}"
                                               data-toggle="toggle"
                                               data-size="mini"
